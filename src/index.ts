                ...
                if (keycloakId) {
                    ...
                    try {
                        kcUserGroups = (
                            ...
                            .map((groupObject) => ({
                                ...
                                slug: (groupObject.attributes as Record<string, any>).workshopSlug[0],
                    ...
                    if (kcUserGroups?.length) {
                        ...
                        for (const kcUserGroup of kcUserGroups) {
                            const existingEntries = workshopGroupsPerSlug.get(kcUserGroup.slug);
                            if (existingEntries) {
                                workshopGroupsPerSlug.set(kcUserGroup.slug, [...existingEntries, kcUserGroup]);
                            } else {
                        ...
                        for (const workshopGroups of workshopGroupsPerSlug.values()) {
                            const userIsOwner = workshopGroups.some(group => group.groupName
                                ?.toLowerCase()
                                .includes(KC_OWNER_SUFFIX));

                            const userIsCrew = workshopGroups.some(group => group.groupName
                                ?.toLowerCase()
                                .includes(KC_CREW_SUFFIX));

                            if (!userIsOwner && !userIsCrew) {
                                logger.debug(
                                    "User is neither owner nor crew member. Not doing anything.", workshopGroups[0].slug
                                );
                                continue;
                            }
                            ...
                            if (userIsOwner) {
                                ...
                                if (spaceExists) {
                                    ...
                                    logger.debug("User is crew, space doesn't exist. Doing nothing.")
                                }
                            }
                        }
                    } else {
            ...
            try {
                ...
                if (
                    ...
                    if (keycloakId) {
                        ...
                        const [cmd, slug] = messageBody.split(' ');
                        const ownerWorkshopGroup = kcUserGroups.find(g => g.slug === slug && g.groupName
                            ?.toLowerCase()
                            .includes(KC_OWNER_SUFFIX));
                        ...
                        if (room?.roomId) {
                            await mxClient.sendTextMessage(
                                ...
                                spaceExists
                                    ? "Dieser Space existiert bereits. Du wurdest erneut eingeladen und zum Admin gemacht."
